...
public class D_UnmuteCommand extends Command {

    private final Bot bot;

    public D_UnmuteCommand(final Bot bot) {
        super("unmute", new HashSet<>());
        this.bot = bot;
    }
                ...
                try {
                    ...
                    CompletableFuture.runAsync(() -> this.bot.getGuardianAPI().isPunished(userId, w -> {
                        if (w) {
                            ...
                            this.bot.getGuardianAPI().unMute(user, sender, server, reason);
